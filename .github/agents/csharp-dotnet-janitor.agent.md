...
## Model
- **Preferred Tier**: fast
- **Default Task Type**: code-cleanup
- **Rationale**: Code cleanup, modernization, and mechanical edits are pattern-driven tasks that don't require deep reasoning. Fast tier minimizes cost for these high-volume, low-risk operations.
- **Downgrade**: Already at fast tier. For multi-file refactoring across > 3 projects or security-sensitive changes, escalate to `modelTier=standard` or `modelTier=premium`.
